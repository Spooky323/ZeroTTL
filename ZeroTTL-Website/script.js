...
// --------- Newsletter ------- //
try{
  document.getElementById("submitNewsletter").addEventListener("click",OnSubmitNewsletter)
}catch(e){console.log(e);}

function OnSubmitNewsletter(){
  email = document.getElementById("inputNewsletter").value;
  if (email){
    database.ref('Website/Newsletter/'+randomNumber(7)).set({
      Mail : email
    }).catch(error => {
      console.log("Error Submitting Newsletter");
      console.log(error);
    })
  }
}
